...
前提文書: [DEVELOPMENT_BRIEF.md](DEVELOPMENT_BRIEF.md)（目的の正本）／[STAGE_DESIGN.md](STAGE_DESIGN.md)（全体設計）／`openspec/changes/add-stage1-agent-core/`（Stage 1 の振る舞い正本）
...
- 旧 `agentic-game-factory` のコードを移植しない。参照は設計パターンのみ（STAGE_DESIGN.md §4）。

## 2. Codex への最初の依頼文（§7.2 準拠・コピーして使う）

```text
docs/DEVELOPMENT_BRIEF.md、docs/STAGE_DESIGN.md、openspec/changes/add-stage1-agent-core/ を読んでください。
...
- レビュー観点:
  ...
  - Agent loop が step ごとに永続化しているか。system prompt が 5–10KB 目安に収まっているか。
...
- タスク分解の目安: 権限強制の実行層検証（path 正規化・コマンド allowlist）→ iframe sandbox の属性固定 → 費用積算と強制停止 → 監査ログの網羅 → 権限逸脱テスト。
...
- レビュー観点: 判断がデータ（集計）に基づいているか。「分離しない」も正当な結論として扱われているか。
